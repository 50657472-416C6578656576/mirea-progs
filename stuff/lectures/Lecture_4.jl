...
Подтипами типа Signed, например, являются типы:
...
Например, параметрическим является абстрактный тип Complex:
    ...
    Complex{Real} # - а это по-прежнему абстрактный тип (т.к. в данном случае параметр - есть абстрактный тип)
...
Кроме того, имеется бинарная операция <: (>:), например:
...
- это было бы неправильным определением типа, правильно в этом случае было бы так:
    Vector{<:Integer}
Для этого правила есть объективные причины.
...
не требовалось бы вносить никакие изменения в это описание.
...
#=
В действительности функция sum является встроенной и её возможная реализация приведена просто в качестве примера.
...
Эти две функции `move!` и `get_coord` составляют интерфейс типа `Coord`.
